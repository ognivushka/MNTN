<template>
  <div class="hero px-20 flex gap-80 justify-end items-center">
    <div class="hero-slider flex flex-col gap-8">
      <div
        v-for="(slide, index) in slidesData"
        :key="index"
        v-show="slide.active"
        :class="[
          `hero-slide-${index + 1}`,
          'max-w-4xl',
          'flex',
          'flex-col',
          'gap-8',
          'overflow-hidden',
        ]"
      >
        <div class="flex gap-3 items-center">
          <div class="w-16 border-t border-accent"></div>
          <p class="uppercase text-accent tracking-[6px]">{{ slide.subtitle }}</p>
        </div>
        <div>
          <h1 class="text-7xl font-headers capitalize">
            {{ slide.title }}
          </h1>
        </div>
      </div>
      <a href="#" class="flex gap-3 items-center">
        <p>scroll down</p>
        <img src="/src/components/icons/arrow_downward.png" alt="arrow down" />
      </a>
    </div>
    <div class="hero_slider-navigation flex flex-col items-end">
      <a href="#" class="pr-6 pb-5 pt-5 border-r">start</a>
      <a href="#" class="pr-6 pb-5 pt-5 border-r">01</a>
      <a href="#" class="pr-6 pb-5 pt-5 border-r">02</a>
      <a href="#" class="pr-6 pb-5 pt-5 border-r">03</a>
      <a href="#" class="pr-6 pb-5 pt-5 border-r hidden">04</a>
    </div>
  </div>
</template>

<script setup lang="ts">
import { defineProps } from "vue";

defineProps({
  slidesData: {
    type: Array,
    required: true,
    default: () => [
      {
        subtitle: "A Hiking guide",
        title: "Be prepared for the Mountains and beyond!",
        active: true,
      },
      {
        subtitle: "A Hiking guide 2",
        title: "Be prepared for the Mountains and beyond!",
        active: false,
      },
      {
        subtitle: "A Hiking guide 3",
        title: "Be prepared for the Mountains and beyond!",
        active: false,
      },
    ],
  },
});
</script>
